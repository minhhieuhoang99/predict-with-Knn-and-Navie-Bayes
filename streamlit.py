...
def accept_user_data():
    ...
    Flight_Distance = st.slider("Khoảng cách chuyến bay:",1,5000)
    Inflight_wifi_service   = st.slider("Đánh giá dịch vụ wifi:",1,5)
    Departure_Arrival_time_convenient   = st.slider ("Đánh giá độ thuận tiện của thời gian đi/đến: ",1,5)
    Ease_of_Online_booking  = st.slider ("Đánh giá dịch vụ đặt vé trực tuyến: ",1,5)
    Gate_location   = st.slider ("Đánh giá vị trí cổng bay: ",1,5)
    Food_and_drink  = st.slider ("Đánh giá đồ ăn/uống: ",1,5)
    Online_boarding = st.slider ("Đánh giá dịch vụ trực tuyến: ",1,5)
    Seat_comfort    = st.slider ("Đánh giá ghế ngồi: ",1,5)
    Inflight_entertainment  = st.slider ("Đánh giá dịch vụ giải trí: ",1,5)
    On_board_service    = st.slider ("Đách giá dịch vụ nhận vé trực tuyến: ",1,5)
    Leg_room_service    = st.slider ("Đánh giá chỗ để chân: ",1,5)
    Baggage_handling    = st.slider ("Đánh giá về hành lý xách tay: ",1,5)
    Checkin_service = st.slider ("Đánh giá dịch vụ checkin: ",1,5)
    Inflight_service    = st.slider ("Đánh giá dịch vụ trên chuyến bay: ",1,5)
    Cleanliness = st.slider ("Đánh giá vệ sinh chuyến bay: ",1,5)
    Departure_Delay_in_Minutes  = st.slider ("Thời gian khởi hành muộn: ",0,500)
    Arrival_Delay_in_Minutes    = st.slider ("Thời gian kết thúc muộn: ",0,500)
